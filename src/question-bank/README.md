...
100～114 年完整題庫可由已校對的 `QuestionInfo/{年度}/{科目}` 與
...
若只需更新特定年度，請在 `--` 後列出年度，避免重寫其他年度已人工校正的內容：

```bash
npm run import:question-bank -- 100 101
```
...
法規題另可人工設定「類似題目細分考點」。使用相同細分考點文字的題目會互相配對；
人工設定優先於關鍵字自動判斷，未設定時才保留自動判斷結果。
